...
Matrix4x4 DirectionToDirection(const Vector3& from,const Vector3& to){
	Vector3 n_from = Normalize(from);
	Vector3 n_to = Normalize(to);
	...
	if(cosTheta > 1.0f - 1.0e-4f){
	...
	else if(cosTheta < -1.0f + 1.0e-4f){
		Vector3 orthogonal_vector = Cross(Vector3(0.0f,0.0f,1.0f),n_from);

		if(Dot(orthogonal_vector,orthogonal_vector) < 1.0e-4f){
			orthogonal_vector = Cross(Vector3(1.0f,0.0f,0.0f),n_from);
		}

		Vector3 axis = Normalize(orthogonal_vector);
		...
		result.m[1][0] = 2.0f * axis.y * axis.x;
		result.m[1][1] = 2.0f * axis.y * axis.y - 1.0f;
		result.m[1][2] = 2.0f * axis.y * axis.z;
		result.m[1][3] = 0.0f;

		result.m[2][0] = 2.0f * axis.z * axis.x;
		result.m[2][1] = 2.0f * axis.z * axis.y;
	...
	// 3. 通常の回転
		...
		Vector3 axis = Normalize(Cross(n_from,n_to));
	...
	for(int r = 0; r < 4; ++r){
		for(int c = 0; c < 4; ++c){
			if(std::abs(result.m[r][c]) < 1.0e-5f){
				result.m[r][c] = 0.0f;
			}
		}
	}
